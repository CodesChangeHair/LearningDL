...
def train_epoch(model, optimizer, output_per_batch: None):
    ...
    for src, tgt in train_dataloader:
        ...
        if output_per_batch is not None and (total_batch - 1) % output_per_batch == 0:
            print("Batch {}: loss: {}, avg loss: {}".format(total_batch, loss.item(), total_loss / total_batch))
...
total_epoch = 10
writer = SummaryWriter("Transformer")
for epoch in range(total_epoch):
    print("Epoch {} / {}".format(epoch + 1, total_epoch))
    train_avg_loss = train_epoch(model, optimizer, 100)
    test_avg_loss = evaluate(model)
    writer.add_scaler("train avg loss", train_avg_loss, epoch)
    writer.add_scaler("test avg loss", test_avg_loss, epoch)
    optimizer.param_groups[0]["lr"] /= 10
    print("Epoch {} done. avg loss = {}".format(epoch, train_avg_loss))
    print("*" * 50)